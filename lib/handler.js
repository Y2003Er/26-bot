...
// ✅ Cache ya group metadata — inatumika na handler NA antilink
const groupMetaCache = new NodeCache({ stdTTL: 300, checkperiod: 60 });

// FIX C-13: Export cache ili antilink.js iweze kuitumia
// badala ya kufanya groupMetadata() fetch yake binafsi
...
function registerCommand(cmd) {
    ...
    // FIX L-6: Ondoa console.log kwa kila command — inajaza logs bila faida
...
export async function loadCommands() {
    ...
    if (fs.existsSync(commandsPath)) {
        ...
        for (const file of files) {
            try {
                // ════════════════════════════════════════════════════════
                // FIX C-3: ONDOA ?t=${Date.now()} — ilikuwa ikisababisha
                // memory leak kubwa. Node.js ESM inacache modules kwa URL
                // yao. Timestamp tofauti kila wakati = nakala mpya ya kila
                // command file kwenye RAM bila kufuta ya zamani.
                // Restart 10 = RAM × 10. Hii ilikuwa sababu kuu ya OOM.
                // ════════════════════════════════════════════════════════
                ...
                if (module.name && typeof module.execute === 'function') {
                    ...
                    if (Array.isArray(module.alias)) {
                        for (const a of module.alias) {
                            if (!a) continue;
                            const aKey = a.toLowerCase();
                            if (!commands.has(aKey)) {
                                commands.set(aKey, {
                                    ...module,
                                    name: a,
                                });
...
function isOwnerJid(jid) {
    if (!jid) return false;
    const clean = j => String(j).split(':')[0].replace(/@lid|@s\.whatsapp\.net|@c\.us/g, '').replace(/[^0-9]/g, '');
...
export async function handleMessage(sock, msg) {
    try {
        ...
        if (isGroup) {
            ...
            try {
                // Tumia cache — fetch tu kama haipo (TTL 300s)
        ...
        for (const msg of messages) {
            ...
            if (isOwnerDM || isGroupChat) {
                ...
                messageCache.set(msg.key.id, {
                    ...
                    mediaType: msg.message?.imageMessage ? 'image' : msg.message?.videoMessage ? 'video' :
                               msg.message?.audioMessage ? 'audio' : msg.message?.stickerMessage ? 'sticker' : null,
                    chatJid,
                    sender,
                    timestamp: Date.now()
                });
            ...
            for (const key of keys) {
                ...
                if (hasMedia) {
                    try {
                        ...
                        if (fullMsg?.message) {
                            const mediaMessage = fullMsg.message.imageMessage || fullMsg.message.videoMessage ||
                                                 fullMsg.message.audioMessage || fullMsg.message.stickerMessage;
                            ...
                            if (mediaMessage) {
                                ...
                                const sendPayload = mediaType === 'image'
                        ...
                        console.error('Anti-delete media reload error:', mediaErr.message);
                    }
                }
            }
        } catch (err) {
...
// ════════════════
// 📊 AUTO STATUS VIEWER + LIKE
// ════════════════
const STATUS_REACTIONS = ['❤️', '🔥', '👍', '😍', '🥰', '💯', '😊', '✨'];
const AUTO_LIKE_REACTION = '❤️';
...
export function setupAutoStatusViewer(sock) {
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            const posterNum = normalizeJid(msg.key.participant || msg.key.remoteJid || '')
            ...
            try {
                ...
                if (global.autoStatusLike) {
                    await sock.sendMessage('status@broadcast', {
                        react: { text: AUTO_LIKE_REACTION, key: msg.key }
                    });
                    console.log(`📊 Status +${posterNum} → viewed ✅ liked ❤️`);
                } else {
                ...
                // FIX M-4: Random delay 3-8s badala ya 2s fixed
                // Inazuia WhatsApp ku-flag account kwa reactions nyingi haraka
    ...
    console.log(`📊 Auto Status Viewer: Imewashwa | Like: ${global.autoStatusLike ? 'ON' : 'OFF'}`);
}