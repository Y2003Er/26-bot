...
if (!process.env.OWNER_NUMBER) {
    console.error('❌ OWNER_NUMBER haijawekwa kwenye.env! Weka OWNER_NUMBER=255XXXXXXXXX kabla ya kuanzisha bot.');
    process.exit(1);
}
...
const groupMetaCache = new NodeCache({ stdTTL: 120, checkperiod: 60 });
...
export async function loadCommands() {
    ...
    if (fs.existsSync(commandsPath)) {
        ...
        for (const file of files) {
            try {
                if (file === 'textmaker.js' &&!textmaker) {
                    console.log(`⚠️ textmaker.js imerukwa - imezimwa kwenye config.js`);
                    continue;
                }
...
function isOwnerJid(jid) {
    ...
    const ownerNums = process.env.OWNER_NUMBER.split(',').map(n => clean(n)).filter(Boolean);
...
export async function handleMessage(sock, msg) {
    try {
        ...
        if (isGroup) {
            ...
            if (!canUseCommands) {
                const pfxMatch = text.trim().match(prefixRegex);
                const pfxCmdName = pfxMatch?.[1]?.toLowerCase() || '';
                const isAiCommand = /^(ai|bot|photo)$/i.test(pfxCmdName);
...
const messageCache = new NodeCache({ stdTTL: 1800, checkperiod: 600, maxKeys: 50});
...
export function setupAntiDelete(sock) {
    ...
    async function sendDeleteAlert(deletedKeyId, chatJidFallback, retriesLeft = 3) {
        let cached = messageCache.get(deletedKeyId);
        if (cached && cached.hasMedia &&!cached.mediaBuffer && retriesLeft > 0) {
            await sleep(1200);
            return sendDeleteAlert(deletedKeyId, chatJidFallback, retriesLeft - 1);
        }
        if (!cached) return;
        messageCache.del(deletedKeyId);
        const { text, hasMedia, mediaType, mediaBuffer, chatJid, sender, isGif } = cached;
        ...
        if (hasMedia && mediaBuffer) {
            const sendPayload =
                ...
                mediaType === 'video'? { video: mediaBuffer, caption: notifText || '🎥 [Video/GIF iliyofutwa]', gifPlayback:!!isGif } :
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            const isGif =!!(vidMsg?.gifPlayback);
            messageCache.set(msg.key.id, {
                text, hasMedia, mediaType, mediaBuffer: null, chatJid, sender, isGif
            });
            if (hasMedia) {
                try {
    ...
    console.log('🛡️ Anti-Delete: ✅ Imewashwa (media inahifadhiwa mapema, max 10MB, retry kwa delay)');
}

// ════════════════
// 📊 AUTO STATUS VIEWER + REACT — INSTANT VIEW (FIXED DUPLICATE)
// ════════════════
...
export function setupAutoStatusViewer(sock) {
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // Skip kama tayari ime-process au iko kwenye queue
            if (viewedStatusCache.has(statusId) || statusProcessingQueue.has(statusId)) {
                continue;
            }

            // Weka kwenye queue KABLA ya ku-process — kuzuia race condition
            statusProcessingQueue.add(statusId);
            
            // Weka kwenye cache KABLA ya ku-view — kuzuia duplicate
            viewedStatusCache.set(statusId, true);

            try {
                // ✅ INSTANT VIEW — hakuna delay
                await sock.readMessages([msg.key]);

                // ✅ REACT instantly after view
                if (autoStatusReact) {
                ...
                // Ikishindwa, futa cache ili iweze kujaribu tena
                viewedStatusCache.del(statusId);
                console.error(`❌ Status error (+${posterNum}):`, err.message);
            } finally {
                // Ondoa kwenye processing queue
                statusProcessingQueue.delete(statusId);
            }
        }
    });

    console.log(`📊 Auto Status Viewer: ✅ Imewashwa | Mode: INSTANT VIEW + ${global.autoStatusLike ? 'LIKE ONLY 🔥' : 'RANDOM REACT'} | Anti-Duplicate ✅`);
}
...
export function setupAutoBio(sock) {
    ...
    updateBio();
    autoBioTimer = setInterval(updateBio, 6 * 60 * 60 * 1000);
    console.log('📝 Auto Bio: Imewashwa (kila masaa 6)');
}
