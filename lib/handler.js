...
export function setupAntiDelete(sock) {
    sock.ev.on('messages.upsert', ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            const isOwnerDM = !isGroupChat && (normalizeJid(chatJid) === normalizeJid(OWNER_ID) || msg.key.fromMe);
...
// ════════════════
// 📊 AUTO STATUS VIEWER + LIKE (FIXED PHYSICAL REACTIONS)
// ════════════════
...
export function setupAutoStatusViewer(sock) {
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            try {
                // 1. Mark status as viewed kwanza kabisa
                await sock.readMessages([msg.key]);

                // 2. PAUSE: Subiri sekunde 3 hadi 6 ili WhatsApp iridhike kuwa umeiona
                // Hii inafanya server isikatae reaction yako, sasa zitaenda physically!
                await sleep(Math.floor(Math.random() * 3000) + 3000);

                // 3. Tuma Reaction sasa baada ya kusubiri
                if (global.autoStatusLike) {
                    await sock.sendMessage('status@broadcast', {
                        react: { text: AUTO_LIKE_REACTION, key: msg.key }
                    });
                    console.log(`📊 Status +${posterNum} → viewed ✅ liked ❤️ physically`);
                } else {
                    const reaction = randomReaction();
                    await sock.sendMessage('status@broadcast', {
                        react: { text: reaction, key: msg.key }
                    });
                    console.log(`📊 Status +${posterNum} → viewed ✅ reacted ${reaction} physically`);
                }
