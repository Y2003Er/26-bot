    ...
    for (const file of files) {
        try {
            ...
            if (cmd.name && typeof cmd.execute === 'function') {
                // ✅ Style 1: normal command with name + execute (e.g. ai.js, ping.js)
                commands.set(cmd.name, cmd);
                console.log(`✅ Command loaded: ${cmd.name}`);
            } else {
                // ✅ Style 2: smd/cmd style (e.g. group.js) — just importing it is enough
                // because smd/cmd register themselves into plugins.js automatically
