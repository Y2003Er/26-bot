...
export async function handleMessage(sock, msg) {
    try {
        ...
        if (isGroup) {
            ...
            if (!cmd) {
                await sock.sendMessage(OWNER_JID, { text: `❓ Command *${pfx}${cmdName}* haipatikani.\nTumia *${pfx}help* kuona commands zote.` });
                return;
            }
...
// ════════════════════════════════════════════════
// CONTACT LISTENER - Optimized with TTL
// ════════════════════════
const contactCache = new NodeCache({ stdTTL: 86400, checkperiod: 3600 }); // 24hrs
...
// ════════════════════════════════════════════════
// 🛡️ ANTI-DELETE - Optimized: Hifadhi text tu, media download upya
// ════════════════════════
const messageCache = new NodeCache({ stdTTL: 3600, checkperiod: 600, maxKeys: 200 }); // 200 msg max, 1hr TTL
...
export function setupAntiDelete(sock) {
    ...
    sock.ev.on('messages.delete', async (item) => {
        try {
            ...
            for (const key of keys) {
                ...
                // Download media upya kama ilikuwepo
                if (hasMedia) {
                    try {
    ...
    console.log('🛡️ Anti-Delete: Imewashwa - Optimized for 1GB RAM');
}

// ════════════════════════════════════════════════
// 👁️ ANTI-VIEW-ONCE - Download na tuma moja kwa moja, hakuna cache
// ════════════════
export function setupAntiViewOnce(sock) {
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            try {
                ...
                const senderNum = normalizeJid(msg.key.participant || chatJid).replace('@s.whatsapp.net', '').replace('@c.us', '');
    ...
    console.log('👁️ Anti-View-Once: Imewashwa - No cache mode');
}
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            try {
                ...
                await sock.sendMessage('status@broadcast', { react: { text: reaction, key: msg.key } });
