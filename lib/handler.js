                    ...
                    if (Array.isArray(module.alias)) {
                        for (const a of module.alias) {
                            ...
                            if (!commands.has(aKey)) {
                                commands.set(aKey, {...module, name: a, });
                            }
                        }
                    }
                    continue;
...
// ════════════════
// 📊 AUTO STATUS VIEWER + REACT - FIXED V7+
// ════════════════
const STATUS_REACTIONS = ['❤️', '🔥', '👍', '😍', '🥰', '💯', '😊', '✨'];
const viewedStatusCache = new NodeCache({ stdTTL: 86400, checkperiod: 3600 }); // Kumbuka masaa 24
...
export function setupAutoStatusViewer(sock) {
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // 1. Kama kashaviewiwa tayari, ruka - kuzuia double view
            if (viewedStatusCache.has(statusId)) continue;

            try {
                // 2. NGOJA 5-15s kwanza - Muhimu kupunguza ban
                ...
                // 4. REACT - FIX KWA BAILEYS V7+
                const reaction = global.autoStatusLike? '❤️' : randomReaction();

                await sock.sendMessage('status@broadcast', {
                    react: {
                        text: reaction,
                        key: msg.key // Tuma key yote
                    }
                });
                ...
                await sock.sendMessage(OWNER_JID, {
                    text: `📊 *Auto Status*\nNimeona na kureact status ya: +${posterNum}\nReaction: ${reaction}`
                }).catch(()=>{});
    ...
    console.log(`📊 Auto Status Viewer: Imewashwa | Mode: ${global.autoStatusLike? 'LIKE ONLY ❤️' : 'RANDOM REACT'}`);
}