...
// ════════════════
// 🛡️ ANTI-DELETE — FIXED CACHE SETTINGS
// ════════════════
const MAX_MEDIA_BYTES = 10 * 1024 * 1024;
const messageCache = new NodeCache({ stdTTL: 300, checkperiod: 60, maxKeys: 500 });
...
export function setupAntiDelete(sock) {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // ✅ Silently skip kama cache imejaa — badala ya ECACHEFULL error
            try {
                messageCache.set(msg.key.id, {
                    text, hasMedia, mediaType, mediaBuffer: null, chatJid, sender, isGif
                });
            } catch (cacheErr) {
                // Cache full — skip silently, message older than TTL will be auto-deleted
            }
    ...
    console.log('🛡️ Anti-Delete: ✅ Imewashwa (media inahifadhiwa mapema, max 10MB, cache: 500 keys/5min TTL)');
}

// ════════════════
// 📊 AUTO STATUS VIEWER + REACT — INSTANT VIEW (FIXED)
// ════════════════
