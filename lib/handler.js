...
const messageCache = new NodeCache({ stdTTL: 3600, checkperiod: 600, maxKeys: 500 });
...
// ════════════════
// 🛡️ ANTI-DELETE FULL FIX (v2 — na REVOKE handling)
// ════════════════
export function setupAntiDelete(sock) {
    if (!ANTIDELETE) return console.log('🛡️ Anti-Delete: Imezimwa');

    // Function moja inayotumika kutuma alert + media, kutoka pande zote mbili
    // (messages.delete event na protocolMessage REVOKE)
    async function sendDeleteAlert(deletedKeyId, chatJidFallback) {
        ...
        if (isGroup) {
            await sock.sendMessage(chatJid, { text: notifText, mentions: [sender] }).catch(async () => {
                await sock.sendMessage(OWNER_JID, { text: notifText + '\n\n_⚠️ Sikurudisha group kwa sababu si admin_' }).catch(() => {});
            });
        } else {
            await sock.sendMessage(OWNER_JID, { text: notifText, mentions: [sender] }).catch(() => {});
        }
                ...
                if (fullMsg?.message) {
                    ...
                    if (mediaMessage) {
                        ...
                        if (buffer) {
                            ...
                            await sock.sendMessage(targetJid, sendPayload).catch(() => {});
                        }
                    }
                }
            } catch (mediaErr) {
    ...
    sock.ev.on('messages.upsert', ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // 🔴 Shika "Delete for Everyone" inayokuja kama protocol message (REVOKE)
    ...
    // Fallback kwa clients zinazotuma delete kama event tofauti badala ya protocolMessage
    ...
    console.log('🛡️ Anti-Delete: ✅ Imewashwa (na REVOKE handling)');
}
