...
export function setupAntiDelete(sock) {
    ...
    async function sendDeleteAlert(deletedKeyId, chatJidFallback, retriesLeft = 3) {
        ...
        // Kama download bado inaendelea (mediaBuffer null lakini hasMedia true), subiri kidogo
    ...
    console.log('🛡️ Anti-Delete: ✅ Imewashwa (media inahifadhiwa mapema, max 8MB, retry kwa delay)');
}
...
export function setupAutoBio(sock) {
    ...
    updateBio(); // fanya mara 1 wakati wa kuanza
    autoBioTimer = setInterval(updateBio, 60 * 60 * 1000); // Kila saa 1
    console.log('📝 Auto Bio: Imewashwa (kila saa 1)');
}
