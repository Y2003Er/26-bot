...
/**
 * Sajili command moja kwenye commands map + global.allCommands
 */
...
export async function loadCommands() {
    ...
    if (fs.existsSync(commandsPath)) {
        ...
        for (const file of files) {
            try {
                ...
                // ── STRATEGY 1: Single command export (ai.js, ping.js, help.js style)
                //    module.name + module.execute → import langine moja
                ...
                // ── STRATEGY 2: Named exports — kila export ni command (group.js style)
                //    export const join = { name, execute, ... }
                let namedLoaded = 0;
                ...
                // ── STRATEGY 3: Default export object/map
                //    export default { join, newgc, ... }  au  export default { name, execute }
                const def = module.default;
                if (def) {
                    // Default ni command moja
                    if (def.name && typeof def.execute === 'function') {
                        registerCommand(def);
                        continue;
                    }
                    // Default ni object wa commands nyingi
    ...
    // ── Plugin commands kutoka plugins.js (smd/cmd style) ──
    try {
...
export async function handleMessage(sock, msg) {
    try {
        ...
        // FIX #1: isOwner — tumia isOwnerJid() tu; fromMe kwenye group si reliable
        const isActualOwner = isOwnerJid(senderJid) || (isGroup && isFromMe);

        // DM kutoka own number (bot ikijiandikia) → IGNORE
        ...
        if (isGroup) {
            ...
            // Angalia kama sender ni admin wa group
            ...
            // Ruhusiwa kutoa commands: Owner AU Admin wa group
            const canUseCommands = isActualOwner || senderIsGroupAdmin;

            // Watu wengine (si owner wala admin): .ai/.bot/.photo TU
            if (!canUseCommands) {
            ...
            // Owner au Admin — lazima prefix au reply
            ...
            // Kama bot si admin na command inahitaji bot kuwa admin (kick, promote n.k.)
            // WhatsApp yenyewe itakataa — tutaarifu owner kwa DM
            try {
...
//
//   🛡️ ANTI-DELETE
//   FIX #4: Cache messages ZOTE za group + DM za owner
//
// ════════════════════════════════════════════════════════════════
...
export function setupAntiDelete(sock) {
    ...
    sock.ev.on('messages.upsert', ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // Cache: DM za owner + messages zote za group
...
//
//   📊 AUTO STATUS VIEWER
//   - Inaona status za kila mtu (readMessages)
//   - Inareact kwa emoji random
//   - HAITUMI kwa owner DM
//
// ════════════════════════════════════════════════════════════════
...
export function setupAutoStatusViewer(sock) {
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            // 1. ── View status (mark kama imeonekana) ──
            try {
            ...
            try {
                ...
                // Baadhi ya contacts hazikubali reaction — skip silently
