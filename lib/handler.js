...
export async function loadCommands() {
    ...
    try {
        ...
        for (const [key, cmd] of pluginCommands.entries()) {
            ...
            // FIX #2 — Plugin execute inawrap handler ili ipokee (sock, msg, args)
            // Group.js handlers zinatarajia (m, text, opts) — tunabuild "m" context ndani
            const executeFunc = async (sock, msg, args) => {
                ...
                if (cmd.handler)  return await cmd.handler(m, text, opts);
                if (cmd.execute)  return await cmd.execute(sock, msg, args);
                if (cmd.func)     return await cmd.func(m, text, opts);
            };
            ...
            commands.set(commandName, {
                ...
                alias:     cmd.alias || [],
                // FIX #6 — adminOnly: kwenye group commands type:"group" inahitaji admin check
                adminOnly: cmd.adminOnly || cmd.type === 'admin' || false,
                needsGroup: cmd.type === 'group' || cmd.category === 'group' || false,
            });
