...
export async function handleMessage(sock, msg) {
    try {
        ...
        // ✅ AUTO_REACT — weka reaction ya kawaida kwenye ujumbe unaopokea
        ...
        if (isGroup) {
            ...
            // ✅ PUBLIC — kama false, owner peke yake anaweza kutumia commands (hata group admin hawezi)
        ...
        const cmdName = match?.[1]?.toLowerCase() || null;
        if (!cmdName) return;
        // ✅ PUBLIC — kama false, DM inapatikana kwa owner peke yake
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            try {
                ...
                // ✅ AUTO_STATUS_REACT — kama false, status inatazamwa tu bila reaction
                if (autoStatusReact) {
...
export function setupAntiTag(sock) {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            if (mentions.length < 5) continue; // kizingiti cha "tag-all" spam
        ...
        for (const call of calls) {
            ...
            try {
                ...
                await sock.sendMessage(callerJid, {
                    ...
                    // Baadhi ya @lid callers hawawezi kupokea DM moja kwa moja — si error mbaya
                });
