...
// ================= WATUMIAJI WALIOWAHI TUMIA PREFIX =================
...
export async function handleMessage(sock, msg) {
    try {
        ...
        // ✅ Prefix detection — .ai na .bot peke yake
        ...
        // ✅ Mwandikishe akitumia prefix
        if (hasPrefix) {
            activatedUsers.add(senderLid);
        }

        let cmdName, args;

        if (hasPrefix) {
            // .ai au .bot — peleka kwa ai command
            cmdName = 'ai';
            args = text.replace(/^\.(ai|bot)\s*/i, '').trim().split(/\s+/);
        } else {
            // Reply — peleka kwa ai command
            cmdName = 'ai';
            args = text.split(/\s+/);
        }
