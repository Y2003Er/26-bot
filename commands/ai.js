...
// FIX D-2: execute() inatumia args kutoka handler — haitegemei fullText pekee
// FIX D-3: Photo/audio/image checks zote zinatumia args kwanza
...
// ✅ FIX D-1: Import prefix kutoka config
...
// ✅ FIX C-5: Pokea existingHistory kama parameter — inaondoa SELECT ya pili
        ...
        try {
            await sock.sendMessage(from, {
                text: '📸 Tuma picha pamoja na command:\n' +
                      `*${pfx}photo blur* — blur\n` +
                      `*${pfx}photo gray* — grayscale\n` +
                      `*${pfx}photo rotate* — rotate 90°\n` +
                      `*${pfx}photo enhance* — resize/sharpen`
            }, { quoted: msg });
        } catch (se) { logger.error('Failed to send photo command guidance:', se.message); }
        return true;
    }

    // ✅ Extract type from commandText
    const photoRegex = getPhotoRegex();
    const type = commandText.replace(photoRegex, '').trim().toLowerCase() || 'enhance';
...
export async function execute(sock, msg, args) {
    try {
        ...
        // ✅ FIX D-3: Pata fullText kutoka msg AU args
        const fullText = (
        ...
        // ═══════════════ PHOTO COMMAND ═══════════════
        const photoRegex = getPhotoRegex();
        const isPhotoCommand = photoRegex.test(fullText) || 
            (args && args[0]?.toLowerCase() === 'photo');
        
        if (isPhotoCommand) {
            const commandText = fullText || (getPrefix() + 'photo ' + (args?.slice(1)?.join(' ') || ''));
            return await handlePhoto(sock, msg, from, commandText);
        }
        ...
        // ═══════════════ TEXT AI ═══════════════
        // ✅ FIX D-3: Tumia args kutoka handler kwanza (tayari imeondoa prefix)
        let query = args?.join(' ')?.trim();

        // Kama hakuna args, angalia kama ni reply kwa bot
        if (!query) {
            ...
            if (isReplyToBot) {
                // Tumia fullText kama query
                const aiRegex = getAiRegex();
                query = fullText.replace(aiRegex, '').trim() || fullText;
            }
        }

        // Kama bado hakuna query, onyesha help
        if (!query) {
            const pfx = getPrefix();
            await sock.sendMessage(from,
        ...
        try {
            ...
            // ✅ FIX C-6: typingDelay max 1200ms
            const typingDelay = Math.min(reply.length * 8, 1200);
            await new Promise(resolve => setTimeout(resolve, typingDelay));

            // ✅ FIX C-5: Pitisha history — inaepuka SELECT ya pili
