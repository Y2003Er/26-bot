    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (upper.endsWith('.RSA') || upper.endsWith('.DSA') || upper.endsWith('.EC')) {
                ...
                const strings = extractStringsFromBuffer(buf, 4).join('\n');
                const cnMatch = strings.match(/CN=([^\n,]+)/);
                const oMatch  = strings.match(/\bO=([^\n,]+)/);
                const ouMatch = strings.match(/OU=([^\n,]+)/);
        ...
        try {
            ...
            async function runAnalysis() {
                ...
                for (const fullPath of allFiles) {
                    ...
                    try {
                        if (ext === '.dex') {
                            ...
                            const arr    = extractStringsFromBuffer(buf);   // array
                            content      = arr.join('\n');                   // string
                        } else if (baseName === 'AndroidManifest.xml') {
                            const buf  = await fs.readFile(fullPath);
                            const text = buf.toString('utf8');
                            content = (text.includes('<?xml') || text.includes('manifest'))
                                ? text
                                : readManifestStrings(buf);                  // array.join inside
                ...
                if (confirmMap.size > 0) {
                    ...
                    const otherConfirm    = all.filter(e =>
                        !isCallback(e) && !isStatus(e) && !isPlatform(e)
                    );
